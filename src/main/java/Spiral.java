...
public class Spiral extends PApplet {
    ...
    public void draw() {
        for (int i = 0; i < PALETTE.length; i++) {
            ...
            for (int k = 0; k < NUMBER_OF_ITERATIONS; k++) {
                stroke(color.red(), color.green(), color.blue(), PALETTE_ALPHA);
                //Start from the point closest to origin among 3 uniformly distributed ones
                PVector point = IntStream.range(0, 3)
                        .mapToObj(j -> new PVector(random(MARGIN, width - MARGIN), random(MARGIN, height - MARGIN)))
                ...
                for (int l = 0; l < LINE_LENGTH; l++) {
                    ...
                    point.add(PVector.fromAngle(TWO_PI * myNoise(point.x, point.y, 100f* i / PALETTE.length))
    ...
    private float myNoise(float x, float y, float z) {
        float scale = NOISE_BASE_SCALE + pow(sq(ORIGIN.x - x) + sq(ORIGIN.y - y), NOISE_EXPONENT);
        return NOISE_EXTERNAL_FACTOR * (int) (NOISE_INTERNAL_FACTOR * noise(x / scale, y / scale, z / scale))
